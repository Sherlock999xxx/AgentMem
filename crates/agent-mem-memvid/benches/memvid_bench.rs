...
mod benchmarks {
    ...
    async fn bench_sequential_writes() {
        ...
        println!("Sequential writes: {} ops in {:?} = {:.2} ops/sec",
    ...
    async fn bench_sequential_reads() {
        ...
        println!("Sequential reads: {} iterations in {:?} = {:.3} ms avg",
                 iterations, duration, avg_latency_ms);

        // Target: <5ms P95 latency
        assert!(avg_latency_ms < 5.0, "Latency above target: {:.3} ms", avg_latency_ms);
    ...
    async fn bench_cache_effectiveness() {
        let config = MemvidConfig::new("bench_cache.mv2")
            .with_cache_size(100);
        let store = MemvidStore::create(config).await.unwrap();
        ...
        println!("Cache effectiveness: first access {:?}, cached access {:?}, speedup: {:.2}x",
                 first_access, cached_access,
    ...
    async fn bench_search_performance() {
        ...
        println!("Search performance: {} iterations in {:?} = {:.3} ms avg",
                 iterations, duration, avg_latency_ms);
