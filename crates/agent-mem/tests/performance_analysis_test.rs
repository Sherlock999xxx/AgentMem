...
async fn test_embedding_performance_bottleneck() {
    ...
    for i in 0..10 {
        let _ = mem
            .add_for_user(format!("{} {}", test_content, i), format!("user-{}", i % 3))
            .await;
    }
    let single_embed_time = start.elapsed();
    println!(
        "单个添加操作（10次串行，包含嵌入）: {:?}, 平均: {:?}",
        single_embed_time,
        single_embed_time / 10
    );
...
async fn test_concurrent_embedding_bottleneck() {
    ...
    println!(
        "  嵌入生成占比: {:.1}%",
        avg_embed_time.as_secs_f64() / (total_time.as_secs_f64() / success_count as f64) * 100.0
    );
    println!(
        "  吞吐量: {:.2} ops/s",
...
async fn test_database_write_performance() {
    ...
    for i in 0..50 {
        ...
        let task = tokio::spawn(async move {
            mem_clone
                .add_for_user(format!("DB write test {}", i), "test-user".to_string())
                .await
        });
        tasks.push(task);
    }
...
async fn test_batch_vs_concurrent_performance() {
    ...
    for i in 0..50 {
        ...
        let task = tokio::spawn(async move {
            mem_clone
                .add_for_user(format!("Concurrent test {}", i), "batch-user".to_string())
                .await
        });
        tasks.push(task);
    }
