...
async fn test_single_vs_batch_performance() {
    ...
    for content in test_contents.iter() {
        match mem
            .add_for_user(content.clone(), "test-user".to_string())
            .await
        {
    ...
    let batch_result = mem
        .add_batch_optimized(batch_contents.clone(), options)
        .await;
...
async fn test_memory_operations_breakdown() {
    ...
    match batch_result {
        Ok(results) => {
            ...
            println!(
                "  批量吞吐量: {:.2} ops/s",
                batch_contents.len() as f64 / batch_time.as_secs_f64()
            );
